...
public class Uncommon {
	  ...
	  if(item1.equalsIgnoreCase("helmet") || item1.equalsIgnoreCase("chestplate") || item1.equalsIgnoreCase("leggings") || item1.equalsIgnoreCase("boots")) {
		  ...
		  armor.setLore(Arrays.asList(new String[] {"§7Health: " + getUncommonHealth(), getTierColor(tier) + "§oˮArmour forged by", getTierColor(tier) + "§o" + getRandomItemLore(), getTierColor(tier) + "Uncommon"}));
		  item.setItemMeta(armor);
		  return getUncommonEnchantment(item, item1);
		  
	  }
	    if(item1.equalsIgnoreCase("axe") || item1.equalsIgnoreCase("sword")) {
    		  ItemMeta armor = item.getItemMeta();
    		  armor.setDisplayName(getTierColor(tier) + getRandomItemName(item1));
    		  armor.setLore(Arrays.asList(new String[] { "§7Damage: " + getUncommonDamage(),getTierColor(tier) + "§oˮWeapon forged by", getTierColor(tier) + "§o" + getRandomItemLore(), getTierColor(tier) + "Uncommon"}));
    		  item.setItemMeta(armor);
    		  return getUncommonEnchantment(item, item1);
	  }
...
public static ItemStack getUncommonEnchantment(ItemStack s, String s1) {
...
public static int getUncommonHealth() {
	  Random r = new Random();
      int Chance = r.nextInt(250);
      int max = r.nextInt(215-100) + 100;
      int lvl5 = r.nextInt(184-100) + 100;
      int lvl3 = r.nextInt(156-100) + 100;
      int lvl2 = r.nextInt(123-100) + 100;
      if(Chance >= 1 && Chance < 50) {
      	return max;
      }
      if(Chance >=50 && Chance < 100) {
...
public static int getUncommonDamage() {
	  Random r = new Random();
      int Chance = r.nextInt(250);
      int max = r.nextInt(43-20) + 20;
      int lvl5 = r.nextInt(39-20) + 20;
      int lvl3 = r.nextInt(31-20) + 20;
      int lvl2 = r.nextInt(24-20) + 20;
      if(Chance >= 1 && Chance < 50) {
      	return max;
      }
      if(Chance >=50 && Chance < 100) {
