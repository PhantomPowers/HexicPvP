...
if(item1.equalsIgnoreCase("helmet") || item1.equalsIgnoreCase("chestplate") || item1.equalsIgnoreCase("leggings") || item1.equalsIgnoreCase("boots")) {
	  ...
	  armor.setLore(Arrays.asList(new String[] { getTierColor(tier) + "§oˮArmour forged in", getTierColor(tier) + "§othe token shopˮ", getTierName(tier)}));
	  item.setItemMeta(armor);
	  return getArmorEnchantment(item, item1);
	  
}
  if(item1.equalsIgnoreCase("axe") || item1.equalsIgnoreCase("sword")) {
	  ItemMeta armor = item.getItemMeta();
	  armor.setDisplayName(getTierColor(tier) + getRandomItemName(item1));
	  armor.setLore(Arrays.asList(new String[] { getTierColor(tier) + "§oˮWeapon forged by", getTierColor(tier) + "§oToken Shop", getTierName(tier)}));
	  item.setItemMeta(armor);
	  return getArmorEnchantment(item, item1);
}
...
public static ItemStack getArmorEnchantment(ItemStack s, String s1) {
